...
bool boundaryCheck(MCGAL::Halfedge* hit) {
    MCGAL::Halfedge* ohit = hit->opposite();
    if (hit->isBoundary() || hit->next()->isBoundary() || hit->next()->next()->isBoundary()) {
        return false;
    }
    if (ohit->isBoundary() || ohit->next()->isBoundary() || ohit->next()->next()->isBoundary()) {
        return false;
    }
    for (MCGAL::Halfedge* h : hit->vertex()->halfedges()) {
        if (h->isBoundary()) {
            return false;
        }
    }
    for (MCGAL::Halfedge* h : hit->end_vertex()->halfedges()) {
        if (h->isBoundary()) {
            return false;
        }
    }
...
void MultiSimpleEncoder::SimplifyVertexTo(size_t uiRemainedVertexNum, int groupId, MCGAL::Halfedge* seed, double dAgressiveness /*=7*/) {
    ...
    for (int iteration = 0; iteration < 100; iteration++) {
        ...
        for (int i = 0; i < MCGAL::contextPool.getHindex(mesh.meshId()); i++) {
            ...
            if (h0->isBoundary() || h0->opposite()->isBoundary() || h0 == seed || h0->next() == seed || h0->next()->next() == seed || h0->opposite() == seed || h0->opposite()->next() == seed ||
                h0->opposite()->next()->next() == seed) {
                continue;
            }
    ...
    // encodeVertexSymbol(groupId);
    dumpToBuffer();
}

bool MultiSimpleEncoder::IsFlipped(MCGAL::Halfedge* edge, const MCGAL::Point& ptTarget) {
...
void MultiSimpleEncoder::Initialize() {
...
double MultiSimpleEncoder::CalculateError(MCGAL::Halfedge* edge, MCGAL::Point& ptResult) {
...
double MultiSimpleEncoder::VertexError(const SymetricMatrix& q, double x, double y, double z) {
    return q[0] * x * x + 2 * q[1] * x * y + 2 * q[2] * x * z + 2 * q[3] * x + q[4] * y * y + 2 * q[5] * y * z + 2 * q[6] * y + q[7] * z * z + 2 * q[8] * z + q[9];
}

void MultiSimpleEncoder::UpdateFaceNormal(MCGAL::Vertex* v0) {}

void MultiSimpleEncoder::UpdateEdgePropertyAroundV(MCGAL::Vertex* v0) {
    double dError = 0;
    MCGAL::Point ptResult;
