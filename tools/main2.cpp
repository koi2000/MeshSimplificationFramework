...
class HalfEdgeMesh {
  ...
  public:
    ...
    bool exportToOFF(const std::string& filename) {
        ...
        // 创建一个映射，将内部顶点ID映射到OFF文件中的顶点索引
        std::unordered_map<int, int> vertexIdToIndex;
        std::vector<Vertex*> validVertices;
        ...
        for (Vertex* v : usedVertices) {
            vertexIdToIndex[v->id] = index++;
            validVertices.push_back(v);
        }

        // 写入OFF头部
        file << "OFF" << std::endl;
        file << validVertices.size() << " " << faces.size() << " 0" << std::endl;
        ...
        for (const auto& face : faces) {
            std::vector<Vertex*> faceVertices = face->getVertices();
            file << faceVertices.size();
            for (Vertex* v : faceVertices) {
                file << " " << vertexIdToIndex[v->id];
            }
            file << std::endl;
        }

        file.close();
        std::cout << "Mesh successfully exported to " << filename << std::endl;
        std::cout << "Exported " << validVertices.size() << " vertices and " << faces.size() << " faces." << std::endl;
        return true;
    }

    // 从OFF文件导入网格
    bool importFromOFF(const std::string& filename) {
        ...
        for (int i = 0; i < numFaces; ++i) {
            ...
            for (int j = 0; j < vertexCount; ++j) {
                int vertexIndex;
                file >> vertexIndex;
                faceVertices.push_back(fileVertices[vertexIndex]);
            }
        ...
        file.close();
        std::cout << "Mesh successfully imported from " << filename << std::endl;
        std::cout << "Imported " << vertices.size() << " vertices and " << faces.size() << " faces." << std::endl;
        return true;
    }
...
int main() {
    ...
    mesh.importFromOFF("../static/untitled.off");

    std::cout << "Original mesh:" << std::endl;
    mesh.printMeshInfo();

    // 导出原始网格
    mesh.exportToOFF("original_cube.off");
    ...
    // 导出简化后的网格
    mesh.exportToOFF("simplified_cube.off");
