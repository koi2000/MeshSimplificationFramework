...
inline string time_string() {
    ...
    char tmbuf[100];
    char buf[256];
    nowtm = localtime(reinterpret_cast<const time_t*>(&tv.tv_sec));
...
inline void tokenize(const std::string& str, std::vector<std::string>& result, const std::string& delimiters = " ,;:\t", const bool keepBlankFields = false, const std::string& quote = "\"\'") {
    // clear the vector
    if (!result.empty()) {
        result.clear();
    }
