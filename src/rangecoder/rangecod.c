...
#ifdef USE_FAKE_FIRST_BYTE
void writeByte(rangecoder *cod, unsigned char x)
{
    if (cod->b_firstByte)
    {
        cod->b_firstByte = 0;
    ...
    *(cod->p_data + *cod->p_dataOffset) = x;
    (*cod->p_dataOffset)++;
}
#   define outbyte(cod,x) writeByte(cod, x);
#else
